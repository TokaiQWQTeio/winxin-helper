...
微信窗口可以被其他程序挡住：已在窗口非前台、未最小化时只读验证 `text` 群的消息控件仍可读取。请保持微信登录，并让助手账号的微信窗口打开 `text` 群。实测缩到系统托盘后，微信进程仍在，但主窗口隐藏，`text` 群控件无法读取；尝试非激活显示窗口仍不可读且可能抢焦点，因此当前版本不支持托盘监听。不可读取期间的消息不会在恢复后补发。若要求微信长期在托盘并且不影响当前桌面，建议在独立电脑或虚拟机里让微信窗口保持打开；同一桌面的网页开关无法绕过客户端隐藏窗口的限制。
