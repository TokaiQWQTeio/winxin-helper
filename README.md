...
启动前页面会要求确认焦点提示。模型生成回复后程序会立即尝试发送，不移动鼠标；微信仍可能短暂抢到焦点，恢复原窗口的效果尚未在持续模式实测。只回复 `text` 白名单群中的真正 @；第三位群成员的识别尚未验证。必须让微信主窗口同时显示目标群消息和左侧会话列表，才能核实真正的 @。控制页会显示该检查是否就绪。默认不会自动启动，也不会因访问网页发送微信消息。
