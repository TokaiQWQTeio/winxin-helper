...
微信窗口可以被其他程序挡住：已在窗口非前台、未最小化时只读验证 `text` 群的消息控件仍可读取。请保持微信登录，并让助手账号的微信窗口打开 `text` 群。最小化到任务栏或缩到托盘时，当前版本不能保证消息控件与后台窗口捕获继续更新；不可读取期间的消息不会在恢复后补发。若需要微信长期最小化运行，应先完成该状态下的真实收发测试；若要求完全不影响当前桌面的焦点，建议将微信和机器人放在独立电脑或虚拟机中。
