...
控制页已支持逐群登记与开关；新群默认关闭。只有 `text` 单群的现有接入经过验证，多群发送仍由程序主动拦截。历史任务的本机存储可记录暂停、续扫、实际日期范围和缺口，旧 @ 不会进入发送队列；微信界面回看尚待本机实测。服务器中继代码只处理在线状态与控制指令，尚未部署；详情见 [本机与服务器部署记录](docs/host-server-deployment.md)。方案已取消虚拟机，助手当前保持停止。
...
启动前页面会要求确认焦点提示。模型生成回复后程序会立即尝试发送，不移动鼠标；微信仍可能短暂抢到焦点，恢复原窗口的效果尚未在持续模式实测。只回复 `text` 白名单群中的真正 @；第三位群成员的识别尚未验证。仅显示群聊消息时，程序结合微信的 U+2005 @ 分隔符和窗口截图 OCR 核实消息及发送者；会话列表可读时继续使用独立的 `[有人@我]` 提示。默认不会自动启动，也不会因访问网页发送微信消息。

微信窗口可以被其他程序挡住：已在窗口非前台、未最小化时只读验证 `text` 群的消息控件仍可读取。请保持微信登录，并让助手账号的微信窗口打开 `text` 群。实测缩到系统托盘后，微信进程仍在，但主窗口隐藏，`text` 群控件无法读取；尝试非激活显示窗口仍不可读且可能抢焦点，因此当前版本不支持托盘监听。不可读取期间的消息不会在恢复后补发。

回复会结合本群已记录的近期消息、长期摘要和微信标题栏显示的当前群人数。机器人确认发送成功后也会把自己的回复记入同群上下文。群人数来自标题栏，例如 `text (3)` 表示当前群共有 3 人；近期发言者人数不能代替群总人数。首次启动前未被程序记录的旧消息不会自动导入。

## 接入与已验证结果

专用助手微信号已在这台电脑登录，并建立了 `text` 测试群。当前微信版本为 4.1.13.65。Windows UI Automation 能读取群标题与消息控件；真正 @ 的消息有 U+2005 分隔符，独立的会话提示还会显示 `[有人@我]`。会话列表可读时程序要求两个信号与发送者、消息正文匹配；只显示群聊时要求 U+2005 标记，并用窗口截图 OCR 核对消息正文与发送者。仍需用第三位成员的新 @ 做实测。程序不读取微信数据库或进程内存。
...
在 `text` 群的单次自动联调成功后，用户授权只为这个测试群开启持续回复。用户随后指出模拟点击会操纵鼠标，因此移除了实际鼠标点击发送。后台投递测试已确认窗口消息方式不移动光标且能发送，但会抢到焦点；网页在启动前显示焦点提示。添加第三位群成员后仍需再做一次真实 @ 验证。
